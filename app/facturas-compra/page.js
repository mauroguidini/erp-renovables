    ...
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      ...
      <div>
        <label className="block text-xs font-medium text-zinc-700">
          Importe total * {!totalTocado && <span className="text-zinc-400">(neto + IVA)</span>}
        </label>
        <input
          required
          type="number"
...
function Factura({ factura, proveedores, categorias, centros, url, onCambio }) {
  ...
  async function handleBorrar() {
    const confirmado = window.confirm(
      "¿Borrar esta factura? También se borra el comprobante si tiene. No se puede deshacer."
    );
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-zinc-900">
            {formatearMonto(factura.importe_total)} · {nombreProveedor ?? "—"}
          </p>
...
export default function FacturasCompra() {
  ...
  // las dos columnas que hacen falta — no toda la fila — sin paginar.
  ...
  async function handleAgregar(e) {
    ...
    if (error) {
      ...
      setErrorGuardado(
        error.code === "23505"
          ? "Ya existe una factura cargada con ese proveedor, tipo y número."
          : error.message
      );
      setGuardando(false);
      return;
    ...
    <div className="min-h-screen bg-zinc-50 p-8 font-sans">
      <div className="mx-auto max-w-4xl">
        ...
        <p className="mt-1 text-sm text-zinc-500">
          Registro interno para no perder facturas. No tiene fines fiscales ni contables.
        </p>
        ...
        >
          <h2 className="text-lg font-semibold text-primary">Nueva factura</h2>
          ...
          >
            {guardando ? "Guardando..." : "Agregar factura"}
          </button>
        </form>
