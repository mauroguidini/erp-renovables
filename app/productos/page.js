"use client";

import { useCallback, useEffect, useState } from "react";
import { supabase } from "@/lib/supabaseClient";
import NuevoProductoForm from "./NuevoProductoForm";
import ImportarProductos from "./ImportarProductos";

export default function Home() {
  const [productos, setProductos] = useState([]);
  const [error, setError] = useState(null);
  const [cargando, setCargando] = useState(true);

  const cargarProductos = useCallback(async () => {
    setCargando(true);
    const { data, error } = await supabase
      .from("productos")
      .select("*")
      .order("nombre");

    if (error) {
      setError(error.message);
    } else {
      setError(null);
      setProductos(data);
    }
    setCargando(false);
  }, []);

  useEffect(() => {
    cargarProductos();
  }, [cargarProductos]);

  return (
    <div className="min-h-screen bg-zinc-50 p-8 font-sans">
      <div className="mx-auto max-w-3xl">
        <h1 className="text-2xl font-semibold text-primary">
          ERP Renovables — Productos
        </h1>

        {cargando && <p className="mt-6 text-zinc-600">Cargando...</p>}

        {error && (
          <div className="mt-6 rounded-lg border border-accent/30 bg-accent/10 p-4 text-accent">
            <p className="font-medium">No se pudo conectar con la base de datos.</p>
            <p className="mt-1 text-sm">
              Error: {error}
            </p>
            <p className="mt-2 text-sm">
              Si el error menciona la tabla <code>productos</code>, seguramente
              todavía falta correr el script SQL en Supabase.
            </p>
          </div>
        )}

        {!cargando && !error && productos.length === 0 && (
          <p className="mt-6 text-zinc-600">
            La conexión funciona, pero todavía no hay productos cargados.
          </p>
        )}

        {!cargando && !error && productos.length > 0 && (
          <ul className="mt-6 divide-y divide-zinc-200 rounded-lg border border-zinc-200 bg-white">
            {productos.map((p) => (
              <li key={p.id} className="flex justify-between px-4 py-3">
                <span className="font-medium text-primary">{p.nombre}</span>
                <span className="text-zinc-500">{p.categoria}</span>
              </li>
            ))}
          </ul>
        )}

        <NuevoProductoForm onProductoCreado={cargarProductos} />
        <ImportarProductos onProductosImportados={cargarProductos} />
      </div>
    </div>
  );
}
